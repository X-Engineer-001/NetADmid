    ...
    const handleAI = () => {
        if(text.trim().length>0&&!AIprocessing){
            ...
            services.user.getAvatar(text).then((gotData)=>{
                if(gotData){
                    setInputs(prev => ({...prev, image: (gotData)}));
                    setText("");
                    setMessage("");
                }else{
    ...
    return (
        <div style={{paddingLeft:"calc(50% - 130px)"}}>
            <div style={{float: "left",textAlign:"left",marginRight:"10px"}}>
                ...
                <br/>
                Password:
                <img src={inputs.image} width="80" height="80" style={{display:"block"}}/>
            ...
            <div style={{textAlign:"left"}}>
                ...
                <input name="ai" maxLength="10" value={text} onChange={handleAIinput}/>
